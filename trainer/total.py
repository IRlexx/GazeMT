...
def main(config):
    ...
    with open(os.path.join(savepath, "train_log"), 'w') as outfile:
        ...
        for epoch in range(1, params.epoch+1):
            ...
            epoch_val_loss = 0
            with torch.no_grad():
                for val_data, val_anno in val_dataset:
                    for key in val_data:
                        if key != 'name': val_data[key] = val_data[key].cuda()
                    val_anno = val_anno.cuda()
                    val_loss = net.loss(val_data, val_anno)
            ...
            log = f"Epoch [{epoch}/{params.epoch}], Train Loss: {epoch_train_loss:.4f}, Val Loss: {epoch_val_loss:.4f}"
            print(log); outfile.write(log + "\n")
    ...
    plt.subplot(1, 3, 3)
    plt.plot(epochs, np.abs(np.array(train_losses) - np.array(val_losses)))
    plt.xlabel('Epoch')
    plt.ylabel('Error')
